...
let createPlaylist = function(trackUri, callback){
  let playlist = null;
  request({
    method: 'POST',
...
let addTracks = function(tracks, playlist, callback){
  let songsUrl = tracks.join(',');
  request({
    method: 'POST',
    // switch out username
    url: (`https://api.spotify.com/v1/users/${user.username}/playlists/` + playlist + '/tracks?position=0&uris=' + songsUrl),
    headers: {
      // needs a proper authorization token
      "Authorization": 'Bearer ' + accessToken,
      'Content-Type': 'application/json'
